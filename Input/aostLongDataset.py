...
class aostLongDataset(d.Dataset):
	...
	def __getitem__(self, index):
		vclips = self.backbone.getitem(index, continous=True)#[64x240x320x3] NOTE: the format is already RGB no need to convert
		...
		scls_id = random.randint(0,3)
		vclips = np.rot90(vclips, scls_id, axes=(1,2))

		tcls_id = random.randint(0,1)
		if tcls_id == 1:
			vclips = vclips[::-1,...]
